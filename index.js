...
// Async function for loading active channels
...
// Async function for saving active channels
...
// Async function with proper error handling and timeout
...
// Optimized command processing with better async handling
async function processShapeApiCommand(guildedMessage, guildedCommandName, baseShapeCommand, requiresArgs = false, commandArgs = []) {
    ...
    // Use Promise.allSettled to handle typing indicator independently
    const [typingResult] = await Promise.allSettled([
        client.rest.put(`/channels/${channelId}/typing`)
    ]);
    
    if (typingResult.status === 'rejected') {
        console.warn(`[Typing Indicator] Error for /${guildedCommandName}:`, typingResult.reason?.message);
    }
...
async function initializeBot() {
    ...
    client.on("messageCreated", async (message) => {
        try {
            ...
            // Handle typing indicator with proper async/await
            try {
