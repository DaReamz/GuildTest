...
function getMediaType(url) {
    ...
    try {
        ...
        // Only allow JPG/JPEG images - filter out other image formats
        if (['.jpg', '.jpeg'].some(ext => pathOnly.endsWith(ext))) return 'image';
        if (['.mp4', '.webm', '.mov'].some(ext => pathOnly.endsWith(ext))) return 'video';
        if (['.mp3', '.ogg', '.wav'].some(ext => pathOnly.endsWith(ext))) return 'audio';
        
        // Log when non-JPG images are filtered out
        if (['.png', '.gif', '.webp', '.bmp'].some(ext => pathOnly.endsWith(ext))) {
            console.log(`[Image Filter] Non-JPG image filtered out: ${url}`);
...
function formatShapeResponseForGuilded(shapeResponse) {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
        ...
        // Only process JPG images, skip filtered images
        if (mediaType === 'image') {
            ...
            break; 
        } else if (mediaType === 'filtered_image') {
            // Remove the filtered image URL from the message content
            console.log(`[Image Filter] Removing non-JPG image from message: ${unwrappedLine}`);
            if (i === lines.length - 1) {
                contentLines = lines.slice(0, i);
            } else {
                contentLines = lines.filter((_, index) => index !== i);
            }
            // Continue processing in case there are other media items
            continue;
    ...
    let messageContent = mediaUrlFoundAndProcessed ? contentLines.join('\n').trim() : shapeResponse;
    const embeds = [];

    if (mediaUrlFoundAndProcessed && mediaUrl) {
        const mediaType = getMediaType(mediaUrl);

        if (mediaType === 'image') {
            embeds.push({ image: { url: mediaUrl } });
            console.log(`[Image Processing] JPG image will be sent: ${mediaUrl}`);
            if (messageContent === "") {
                return { embeds };
            }
    ...
    // If we filtered out images but there's still content, return just the text
    if (contentLines.length > 0 && !mediaUrlFoundAndProcessed) {
...
client.on("ready", () => {
    ...
    console.log(`[Image Filter] Bot configured to only send JPG/JPEG images`);
});
